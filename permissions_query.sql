SET NOCOUNT ON
print '---START list of users not sysadmins with CREATE, DROP, ALTER---'
...
WHERE P.permission_name in ('CREATE', 'DROP', 'ALTER')
and DP.name not in (select name from master.dbo.syslogins where sysadmin = 1)

print '----END list of users not sysadmins with CREATE, DROP, ALTER----'

print ''
print ''
print ''

if exists (select name from sysobjects where name = 'tempzzz') drop table tempzzz
print '---START List of users in database with db_owner or db_ddladmin or db_accessadmin or db_securityadmin----------'
create table tempzzz(DBRole varchar(100), MemberName varchar(100), MemberSID varchar(1000))
insert tempzzz EXEC sp_helprolemember
select MemberName from tempzzz
where DBRole in ('db_owner', 'db_ddladmin', 'db_accessadmin', 'db_securityadmin')
print '---END List of users in database with db_owner or db_ddladmin or db_accessadmin or db_securityadmin----------'
if exists (select name from sysobjects where name = 'tempzzz') drop table tempzzz